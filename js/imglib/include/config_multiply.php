 ...
 * Initialize config
 *
 * You can use multiply configuration by create the $CFG['config_name'] = array(overwriten_config_parameter_name => value)
 * The new configuration overwrite the default configuration rules, so if in new configuration you neen only change some one parameters you dont need to copy all default configuration, only the changed values.
 *
 *
 * Example: create new configuration with limit of uploaded file size 300kb:
 * $CFG['limitUploadSize'] = array('maxUploadFileSize' => '300k');
 *
 * You can access to new configuration by set the 'getConfig' parameter with values of configuration name.
 * For previsious example imglib.php?getConfig=limitUploadSize
 *
 * If you want load default config use getConfig=default or not select the config name by use imglib.php?getConfig
 *
 */
$CFG = array();
 ...
 */
$CFG['default']['fileExt'] = 'gif|png|jpeg|jpg|bmp';

/*
 * Format who support the creating of thumbnail
 */
 $CFG['default']['thmbFileExt'] = 'gif|png|jpeg|jpg';
...
*/
$CFG['default']['maxUploadFileSize'] = 0;
 ...
 * Examples:
 * 1) $CFG['default']['fileNameSuffix'] = '_copy_of_Y.m.d_H.i.s_[%1$d]'; - create copy file of file "my_file.jpg" with date and time of copying and iteration number in name like "my_file_copy_of_2010.02.14_10.44.56_[1].jpg"
 * 2) $CFG['default']['fileNameSuffix'] = '_copy_of_Y.m.d_H.i.s'; - create copy file of file "my_file.jpg" with with date and time of copying in name like "my_file_copy_of_2010.02.14_10.44.56.jpg"
 */
$CFG['default']['fileNameSuffix'] = '_[%1$d]';
 ...
 */
$CFG['default']['overwriteFile'] = false;
 ...
 */
$CFG['default']['historyNavigation'] = true;
 ...
 */
$CFG['default']['allowCreateDir'] = true;
 ...
 */
$CFG['default']['allowFileCopy'] = ($CFG['default']['browseSubDir']) ? true : false;

/*
 * Image quality 0-100 for JPEG and PNG files on save
 */
$CFG['default']['imgQuality'] = 80;
 ...
 *
 */
$CFG['default']['thmbRelocating'] = false;
$CFG['default']['thmbSaveDir'] = '../upload/img_thmb/';
//$CFG['default']['thmbSaveURL'] = '/upload/img_thmb';
...
/*
 * Simple user defined config
 */

/*
 * Initialise the user defined config
 */
$CFG['noModules'] = array();
 ...
 * Disable the modules
 */
$CFG['noModules']['modules'] = array();

$CFG['noModules']['uploadNameFormat'] = 'Y_m_d_H_i_s';
...
/*
 * Select the active configuration name from the $_GET['getConfig'] or $_GET['setConfig']
 * If selected configuration not found - then use default
 */
if (isset($_GET['setConfig'])) {
	$activeConfigName = urldecode($_GET['setConfig']);
} else if (isset($_GET['getConfig'])) {
	$activeConfigName = urldecode($_GET['getConfig']);
}

if (isset($activeConfigName) && !empty($activeConfigName) && ($activeConfigName !== 'default') && isset($CFG[$activeConfigName])&& !empty($CFG[$activeConfigName]) && is_array($CFG[$activeConfigName])) {
	// Save the active config name
	$activeConfig = $CFG[$activeConfigName];

	// Copy all not overwriten options from default config
	foreach($CFG['default'] as $key => $val) {
		if (!isset($activeConfig[$key])) {
			$activeConfig[$key] = $val;
		}
	}
} else {
	// Use default config if selected not found or empty
	$activeConfig = $CFG['default'];
	$activeConfigName = 'default';
}

// Replace the config list to active config
$CFG = $activeConfig;

/*
 * Save the active config name
 */
$CFG['configName'] = $activeConfigName;
//header('X-config-name: '. $activeConfigName);

unset($activeConfig, $activeConfigName);
