...
var imageTools = (function () {
	var
		ImgToolsOptions = {
			uploadPath: undefined, // Path to uplad folder
			action: '../../imglib.php?module=imgtools', // URL to file who process the image edit and preview
		...
		imageSrc, // The full path to image in upload folder
		configName //The name of current configuration
	;
	...
	*/
		...
		var
			configScript,
			firstScript = document.getElementsByTagName('script')[0] // Link to first script on page
		;
		imageSrc = window.location.href, // Create a short link to location href
		imageSrc = imageSrc.substring(imageSrc.indexOf('#') + 1).split('|')
		configName = (imageSrc.length > 1) ? (imageSrc[1]) : configName;
		imageSrc = decodeURI(imageSrc[0]);
		

		// Check the source data is aviable
		...
		// Apply the current configuration name
		if (!!configName) {
			ImgToolsOptions.action += '&setConfig=configName';
		}

		// Create script to load configuration
		firstScript.parentNode.insertBefore(createElement('script', {type: 'text/javascript', async: true, src: ('../../imglib.php?getConfig' + ((!!configName) ? ('=' + configName) : '' ) + '&module=imgtools')}), firstScript);
	}
	...
	*/
		...
		addEvent($('rotate_ccw'), 'click', function (evt) {previewRotateFlip(0, -90); cancelEvent(evt);});
		addEvent($('rotate_cw'), 'click', function (evt) {previewRotateFlip(0, 90); cancelEvent(evt);});
	...
	function applyConfig (config) {
		ImgToolsOptions = extend(ImgToolsOptions, config || {});
		ImgToolsOptions.src = ImgToolsOptions.uploadPath + getURIEncPath(imageSrc);
	...
	function applyCrop () {
		...
		if ( ((newCrop.X2 - newCrop.X1) > 0) || ((newCrop.Y2 - newCrop.Y1) > 0) ) {
			...
			var params = 'cmd=edit&' + ImgToolsOptions.srcArg + '=' + encodeURIComponent(imageSrc) + '&' + ImgToolsOptions.dstArg + '=' + encodeURIComponent(imageSrc) + '&' + ImgToolsOptions.cropArg + '=' + newCrop.X1 + ',' + newCrop.Y1 + ',' + (newCrop.X2 - newCrop.X1) + ',' + (newCrop.Y2 - newCrop.Y1);
	...
	function applyResize () {
		...
		if ( (newWidth > 0 && newHeight > 0) && ( (newWidth != imgWidth) || (newHeight != imgHeight) ) ) {
			...
			var params = 'cmd=edit&' + ImgToolsOptions.srcArg + '=' + encodeURIComponent(imageSrc) + '&' + ImgToolsOptions.dstArg + '=' + encodeURIComponent(imageSrc) + '&' + ImgToolsOptions.resizeArg + '=' + newWidth + ',' + newHeight;
			sendXMLHttpReq(ImgToolsOptions.action, {
				mode: 'POST',
				parameters: params,
				onsuccess: function(req) {
	...
	function applyRotateFlip () {
		...
		if ( !( ( (angle == 0) && (flipType == -1) ) || ( (angle == 180) && (flipType == 2) ) ) ) {
			...
			var params = 'cmd=edit&' + ImgToolsOptions.srcArg + '=' + encodeURIComponent(imageSrc) + '&' + ImgToolsOptions.dstArg + '=' + encodeURIComponent(imageSrc) + '&' + ImgToolsOptions.rotateArg + '=' + angle + '&' + ImgToolsOptions.flipArg + '=' + flipType;
			sendXMLHttpReq(ImgToolsOptions.action, {
				mode: 'POST',
				parameters: params,
				onsuccess: function(req) {
	...
	return {
		init: function (config) {
			// Apply the configuration of the Image Tools
			applyConfig(config);

			// Initialise Image Tools
			initImageTools();
		},
		loadConfig: function () {
			/*
				Load the configuration of Image Tools component
			*/
			addEvent(window, 'load', loadConfiguration);
		}
}
}());

imageTools.loadConfig();
